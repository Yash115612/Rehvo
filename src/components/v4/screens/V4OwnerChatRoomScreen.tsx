...
import { V4OwnerQuickReplies } from '../chat/V4OwnerQuickReplies';
...
export const V4OwnerChatRoomScreen: React.FC = () => {
  ...
  const activeId = id || 'conv-lead-101';
  ...
  const [visitSlot, setVisitSlot] = useState('5:00 PM');
  const [rentAmount, setRentAmount] = useState(
    conversation?.property_rent?.toString() || '75000'
  );
  const [agreementTerm, setAgreementTerm] = useState('11 Months (Oct 2026 - Sep 2027)');

  // Reply State
  const [replyTo, setReplyTo] = useState<ChatReplyTo | null>(null);
  const [isPropertyPinned, setIsPropertyPinned] = useState(true);
  ...
  useEffect(() => {
    ...
    const { unsubscribe: unsubTyping } = chatService.subscribeToTyping(
      conversation.id,
      (typingUsers) => {
        const tenantTyping = typingUsers.some(
          (u) => u.userId !== user?.id && u.isTyping
        );
        setIsTenantTyping(tenantTyping);
      }
    );

    // Subscribe to presence
    const effectiveUserId = user?.id || 'host-user-1';
    const effectiveUserName = user?.name || 'Owner';
    const { unsubscribe: unsubPresence } = chatService.subscribeToPresence(
      ...
      (presenceList) => {
        ...
        setIsTenantOnline(otherPresent);
      }
    );
  ...
  const handleSendVisitInvite = () => {
    if (!conversation) return;
    sendRichMessage(conversation.id, {
      text: `Hi ${conversation.other_user_name}, I would like to invite you for a physical site visit on ${visitDate} at ${visitSlot}.`,
      messageType: 'visit',
      metadata: {
        visit: {
    ...
    showToast?.('Visit invitation dispatched with QR Entry pass', 'success');
    setTimeout(() => scrollViewRef.current?.scrollToEnd({ animated: true }), 100);
  };

  const handleSendAgreement = () => {
    if (!conversation) return;
    const rentNum = parseInt(rentAmount) || conversation.property_rent || 75000;
    sendRichMessage(conversation.id, {
      text: `Here is the official 11-month state-stamped digital rental agreement for ${conversation.property_title}. Please review and sign.`,
      messageType: 'agreement',
      metadata: {
        agreement: {
          title: '11-Month Registered Rental Agreement',
          property_id: conversation.property_id,
          property_title: conversation.property_title,
          lease_term: agreementTerm,
          monthly_rent: rentNum,
          security_deposit: rentNum * 2,
          status: 'pending_signature',
          stamp_duty_verified: true,
        },
      },
    });
    setIsAgreementModalOpen(false);
    showToast?.('Digital agreement dispatched for tenant review', 'success');
    setTimeout(() => scrollViewRef.current?.scrollToEnd({ animated: true }), 100);
  };

  const handleSendRentReminder = () => {
    if (!conversation) return;
    const rentNum = parseInt(rentAmount) || conversation.property_rent || 75000;
    sendRichMessage(conversation.id, {
      text: `Rent collection reminder for ${conversation.property_title}: ₹${rentNum.toLocaleString('en-IN')} is due on the 5th.`,
    ...
    showToast?.('Rent collection reminder dispatched', 'success');
    setTimeout(() => scrollViewRef.current?.scrollToEnd({ animated: true }), 100);
  };
    ...
    sendRichMessage(conversation.id, {
      ...
      metadata: {
        property: {
          ...
          title: conversation.property_title || 'Luxury Penthouse',
  ...
  const handlePickVideo = async () => {
    ...
    try {
      ...
      if (!result.canceled && result.assets?.[0]?.uri) {
        showToast?.('Uploading property video walkthrough...', 'info');
        const uploadRes = await uploadChatAttachment(result.assets[0].uri);
        ...
        showToast?.('Video tour sent', 'success');
        setTimeout(() => scrollViewRef.current?.scrollToEnd({ animated: true }), 100);
      }
    } catch (_) {
  ...
  const handleSendDocument = () => {
    ...
    if (!conversation) return;
    sendRichMessage(conversation.id, {
      text: '📄 Document: Property_Title_Deed_NOC.pdf',
      messageType: 'document',
      documentUrl: 'https://assets.rehvo.com/documents/Property_Title_Deed_NOC.pdf',
      documentName: 'Property_Title_Deed_NOC.pdf',
    });
  ...
  const handleShareLocation = () => {
    setIsAttachmentsModalOpen(false);
    if (!conversation) return;
    const locName = conversation.property_locality || conversation.property_title || 'Property Society Location';
    sendRichMessage(conversation.id, {
  ...
  const chatMessages: V4ChatMessage[] = useMemo(() => {
    ...
    return conversation.messages
      ...
      .map((m) => {
        const isMe = m.sender_id === user?.id || m.sender_id === 'host-user-1' || m.sender_id === 'me';
  ...
  return (
    ...
    >
      ...
      <View style={[styles.header, { paddingTop: Math.max(insets.top, 12) }]}>
        <View style={styles.headerRow}>
          ...
          >
            <View style={styles.headerAvatarWrap}>
              ...
              <View
                style={[
                  ...
                  !(isTenantOnline || conversation.is_online) && { backgroundColor: '#94A3B8' },
                ]}
              />
            </View>
            ...
            <View style={styles.headerTextCol}>
              ...
              <Text
                style={[
                  styles.headerRole,
                  (isTenantTyping || isTenantOnline) && { color: '#0F766E', fontWeight: '700' },
                ]}
                numberOfLines={1}
              >
                {isTenantTyping
                  ...
                  : isTenantOnline
                  ? '● Online'
                  : `${conversation.other_user_role || 'VERIFIED TENANT'} • Active recently`}
              </Text>
            </View>
          </Pressable>
          ...
          <View style={styles.headerActions}>
            ...
            <Pressable
              style={styles.headerActionBtn}
              onPress={() => showToast?.('HD 1-on-1 Video Tours coming in REHVO V4.7', 'info')}
              accessibilityLabel="Video tour"
            >
              <Video size={16} color="#0F766E" strokeWidth={2.4} />
            </Pressable>
        ...
        {conversation.property_title && isPropertyPinned && (
          <View style={styles.pinnedPropertyBar}>
            ...
            >
              <Calendar size={12} color="#FFFFFF" strokeWidth={2.4} />
              <Text style={styles.pinnedActionBtnText}>Invite Visit</Text>
            </Pressable>
          </View>
        )}
      </View>
          ...
          return (
            <React.Fragment key={msg.id}>
              ...
              <V4ChatBubble
                ...
                onAgreementCardPress={() => setIsAgreementModalOpen(true)}
                onRentReminderPress={() => setIsRentModalOpen(true)}
              />
            </React.Fragment>
          );
        })}
      ...
      {/* 4. SMART OWNER TOOLS (Quick replies) */}
      <V4OwnerQuickReplies
        propertyName={conversation.property_title || 'the flat'}
        onSelectReply={handleSendText}
        onSendVisitInvite={() => setIsVisitModalOpen(true)}
        onSendAgreement={() => setIsAgreementModalOpen(true)}
        onSendRentReminder={() => setIsRentModalOpen(true)}
      />
      ...
      <V4MessageMenu
        ...
        isMe={
          ...
          selectedMessage?.sender_id === 'host-user-1' ||
        ...
        <Pressable style={styles.modalBackdrop} onPress={() => setIsCallModalOpen(false)}>
          <View style={styles.modalSheet}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Connect with Verified Tenant</Text>
            ...
            <Pressable
              ...
              onPress={() => {
                ...
                Linking.openURL('tel:+919821099882');
              }}
            >
              <Phone size={16} color="#FFFFFF" strokeWidth={2.4} />
              <Text style={styles.modalPrimaryBtnText}>Call Masked Number (+91 98210 ...)</Text>
            </Pressable>
          </View>
        </Pressable>
      </Modal>

      {/* 9. SCHEDULE VISIT MODAL */}
      <Modal visible={isVisitModalOpen} transparent animationType="slide">
        <Pressable style={styles.modalBackdrop} onPress={() => setIsVisitModalOpen(false)}>
          <View style={styles.modalSheet}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Invite Tenant for Site Visit</Text>
              <Pressable onPress={() => setIsVisitModalOpen(false)}>
                <X size={18} color="#0F172A" />
              </Pressable>
            </View>

            <Text style={styles.modalSub}>Select proposed tour day:</Text>
            <View style={styles.pillRow}>
              {['Today', 'Tomorrow', 'This Saturday', 'This Sunday'].map((d) => (
                <Pressable
                  key={d}
                  style={[styles.modalPill, visitDate === d && styles.modalPillActive]}
                  onPress={() => setVisitDate(d)}
                >
                  <Text style={[styles.modalPillText, visitDate === d && styles.modalPillTextActive]}>
                    {d}
                  </Text>
                </Pressable>
              ))}
            ...
            <View style={styles.pillRow}>
              {['11:00 AM', '2:00 PM', '5:00 PM', '7:00 PM'].map((s) => (
                <Pressable
                  key={s}
                  style={[styles.modalPill, visitSlot === s && styles.modalPillActive]}
                  onPress={() => setVisitSlot(s)}
                >
                  <Text style={[styles.modalPillText, visitSlot === s && styles.modalPillTextActive]}>
                    {s}
                  </Text>
                </Pressable>
              ))}
            ...
            <Pressable style={styles.modalPrimaryBtn} onPress={handleSendVisitInvite}>
              <Calendar size={16} color="#FFFFFF" strokeWidth={2.4} />
              <Text style={styles.modalPrimaryBtnText}>Dispatch Site Tour Invitation</Text>
            </Pressable>
          </View>
        </Pressable>
      </Modal>

      {/* 10. SEND AGREEMENT MODAL */}
      <Modal visible={isAgreementModalOpen} transparent animationType="slide">
        <Pressable style={styles.modalBackdrop} onPress={() => setIsAgreementModalOpen(false)}>
          <View style={styles.modalSheet}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Dispatch Digital Lease Agreement</Text>
              <Pressable onPress={() => setIsAgreementModalOpen(false)}>
                <X size={18} color="#0F172A" />
              </Pressable>
            </View>

            <Text style={styles.modalSub}>Monthly Rent (₹):</Text>
            <View style={styles.inputBox}>
              <IndianRupee size={16} color="#0F766E" />
              <TextInput
            ...
            <Text style={[styles.modalSub, { marginTop: 12 }]}>Agreement Duration:</Text>
            <View style={styles.inputBox}>
              <Calendar size={16} color="#0F766E" />
              <TextInput
                style={styles.modalTextInput}
                value={agreementTerm}
                onChangeText={setAgreementTerm}
              />
            </View>
            ...
            <View style={styles.stampNotice}>
              ...
              <Text style={styles.stampNoticeText}>
                Includes government digital e-stamp and DigiLocker Aadhaar e-Sign.
              </Text>
            </View>

            <Pressable style={styles.modalPrimaryBtn} onPress={handleSendAgreement}>
              <FileText size={16} color="#FFFFFF" strokeWidth={2.4} />
              <Text style={styles.modalPrimaryBtnText}>Send Agreement for Signature</Text>
            </Pressable>
          </View>
        </Pressable>
      </Modal>

      {/* 11. SEND RENT REMINDER MODAL */}
      <Modal visible={isRentModalOpen} transparent animationType="slide">
        <Pressable style={styles.modalBackdrop} onPress={() => setIsRentModalOpen(false)}>
          <View style={styles.modalSheet}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Send Rent Collection Notice</Text>
              <Pressable onPress={() => setIsRentModalOpen(false)}>
                <X size={18} color="#0F172A" />
              </Pressable>
            </View>

            <Text style={styles.modalSub}>Amount Due (₹):</Text>
            <View style={styles.inputBox}>
              <IndianRupee size={16} color="#0F766E" />
              <TextInput
                style={styles.modalTextInput}
            ...
            <Pressable style={styles.modalPrimaryBtn} onPress={handleSendRentReminder}>
              <IndianRupee size={16} color="#FFFFFF" strokeWidth={2.4} />
              <Text style={styles.modalPrimaryBtnText}>Dispatch Rent Reminder</Text>
            </Pressable>
          </View>
        </Pressable>
      </Modal>
          ...
          <View style={styles.menuSheet}>
            ...
            <Pressable
              ...
              onPress={() => {
                ...
                showToast?.('Conversation notification preferences updated', 'info');
              }}
            >
            ...
            <Pressable
              ...
              onPress={() => {
                setIsMoreMenuOpen(false);
                Alert.alert(
                  'Block Tenant & Report Spam',
                  'Are you sure you want to block this tenant? They will no longer be able to message you or view your listings.',
                  [
                    { text: 'Cancel', style: 'cancel' },
                    {
                      ...
                      onPress: () => {
                        ...
                        showToast?.('Tenant blocked and reported to trust & safety team', 'success');
                        router.back();
                      },
                    },
                  ]
      ...
      {/* 13. CONVERSATION MEDIA GALLERY MODAL */}
      {conversation && (
...
const styles = StyleSheet.create({
  ...
  root: {
    flex: 1,
    backgroundColor: '#FAF8F5', // Warm Ivory Base
  },
  header: {
    backgroundColor: '#FFFFFF',
    borderBottomWidth: 1,
