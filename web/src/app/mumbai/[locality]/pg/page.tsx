...
export async function generateMetadata({ params }: PgPageProps): Promise<Metadata> {
  ...
  return constructSeoMetadata({
    title: `PG in ${localityName}, Mumbai | Zero Brokerage Paying Guest Accommodations`,
    description: `Find verified PG, co-living, and paying guest accommodations in ${localityName}, Mumbai. Zero brokerage, transparent deposits, and direct host bookings on REHVO.`,
    canonicalUrl: `https://rehvo.com/mumbai/${localitySlug}/pg`,
  });
}

export default async function LocalityPgPage({ params }: PgPageProps) {
  const localitySlug = params.locality.toLowerCase();
  const localityInfo = MUMBAI_LOCALITIES[localitySlug];
  const localityName = localityInfo ? localityInfo.name : unslugify(localitySlug);
  ...
  const breadcrumbs = [
    ...
    { name: 'PG Accommodations', url: `/mumbai/${localitySlug}/pg` },
  ];

  const breadcrumbSchema = generateBreadcrumbSchema(breadcrumbs);
  const itemListSchema = generateItemListSchema(
    `PG Accommodations in ${localityName}, Mumbai`,
    properties.map((p) => ({
      name: p.title,
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-10 border border-stone-200 shadow-sm mt-4 mb-10">
          <div className="max-w-3xl">
            ...
            <h1 className="text-2xl sm:text-4xl font-extrabold text-stone-900 tracking-tight">
              PG in {localityName}, Mumbai
            </h1>
            <p className="text-sm sm:text-base text-stone-600 mt-2 leading-relaxed">
              Find verified paying guest and student accommodations in {localityName} with zero brokerage and verified security deposits.
            </p>
          </div>
        </div>
          ...
          {properties.length > 0 ? (
            ...
            <div className="bg-white rounded-2xl p-12 text-center border border-stone-200">
              ...
              <h3 className="text-base font-bold text-stone-800">
                No active PGs in {localityName} right now
              </h3>
              <p className="text-xs text-stone-500 max-w-md mx-auto mt-1 mb-6">
                Explore all verified PG & co-living options across Mumbai or browse rooms in {localityName}.
              </p>
              <Link
                href="/pg/mumbai"
