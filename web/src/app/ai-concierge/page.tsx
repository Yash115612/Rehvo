    ...
    <div className="min-h-screen bg-[#F8FAFC] py-8 sm:py-12">
      <div className="max-w-[1360px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto my-12 space-y-4">
          <div className="inline-flex items-center gap-2 bg-[#CCFBF1] text-[#064E3B] border border-[#0F766E]/20 px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-wider">
            <Sparkles className="w-4 h-4 text-[#0F766E]" />
            <span>AI CONCIERGE & DECISION ENGINE</span>
          </div>

          <h1 className="text-3xl sm:text-5xl lg:text-6xl font-black text-[#031B2A] tracking-tight leading-tight">
          ...
          <div className="pt-4 flex items-center justify-center gap-3">
            ...
            <Link
              href="/search"
              className="h-12 px-6 rounded-full bg-white hover:bg-[#F1F5F9] border border-[#E2E8F0] text-xs font-bold text-[#031B2A] flex items-center gap-2 transition"
            >
        ...
        <div className="my-16 space-y-6 max-w-4xl mx-auto">
          <div className="text-center space-y-1 mb-8">
            <h2 className="text-2xl font-black text-[#031B2A]">See How REHVO AI Works</h2>
          ...
          <div className="space-y-6">
            {CONVERSATION_EXAMPLES.map((conv, idx) => (
              <div
                key={idx}
                className="bg-white rounded-[28px] p-6 sm:p-8 border border-[#E2E8F0] shadow-card space-y-4"
              >
                {/* User Prompt */}
                <div className="flex items-start gap-3">
                  <div className="w-8 h-8 rounded-xl bg-[#F1F5F9] text-[#031B2A] flex items-center justify-center text-xs font-black shrink-0">
                    You
                  </div>
                  <div className="bg-[#F8FAFC] p-3.5 rounded-2xl text-xs sm:text-sm font-bold text-[#031B2A] border border-[#E2E8F0] flex-1">
                ...
                <div className="flex items-start gap-3 pl-6 sm:pl-10">
                  <div className="w-8 h-8 rounded-xl bg-[#CCFBF1] text-[#0F766E] flex items-center justify-center shrink-0">
                    <Sparkles className="w-4 h-4" />
                  </div>
                  <div className="bg-gradient-to-br from-[#0F766E]/5 to-transparent p-4 rounded-2xl text-xs sm:text-sm text-[#031B2A] border border-[#0F766E]/20 flex-1 space-y-3">
                    <p className="font-medium leading-relaxed">{conv.aiResponse}</p>
                    <div className="flex flex-wrap gap-2 pt-1">
                      {conv.matchPills.map((pill, pIdx) => (
                        <span
                          key={pIdx}
                          className="text-[10px] font-black px-2.5 py-0.5 rounded-md bg-white border border-[#E2E8F0] text-[#0F766E]"
                        >
                          ✓ {pill}
                        </span>
                      ))}
